...
do
  ...
  mkdir /pnfs/uboone/scratch/users/$USER_NAME/$project'_'$nuance/pndr/$index
  
  # Make a submission script for this file
  cp $WORKING_DIR/generic/sh/job_grid_pndr_cosmic.sh $WORKING_DIR/projects/$project/$nuance/job_grid_pndr_cosmic_${project}_${nuance}_${index}.sh
  sed -i -e 's,USER_NAME,'`echo $USER_NAME`',g' $WORKING_DIR/projects/$project/$nuance/job_grid_pndr_cosmic_${project}_${nuance}_${index}.sh
  sed -i -e 's,WORKING_DIR,'`echo $WORKING_DIR`',g' $WORKING_DIR/projects/$project/$nuance/job_grid_pndr_cosmic_${project}_${nuance}_${index}.sh
  sed -i -e 's,PROJECT,'`echo $project`',g' $WORKING_DIR/projects/$project/$nuance/job_grid_pndr_cosmic_${project}_${nuance}_${index}.sh
